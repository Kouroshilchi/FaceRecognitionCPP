...
try:
    model_path = '../models/torchscript.pt'  
    if not os.path.exists(model_path):
        model_path = 'torchscript.pt'
    if not os.path.exists(model_path):
...
def get_embedding(face_img):
    ...
    try:
        ...
        face_tensor = transform(face_pil).unsqueeze(0).to(device)  # (1, 3, 224, 224)

        with torch.no_grad():
            embedding = model(face_tensor)  # (1, 128)
...
def compare_embedding(emb, threshold=0.7, k=5):
    ...
    # نرمالایز برای cosine similarity با IndexFlatIP
    ...
    for dist, idx in zip(distances[0], indices[0]):
        ...
        if name in class_similarities:
            class_similarities[name].append(dist)
        else:
            class_similarities[name] = [dist]
    ...
    if best_avg_sim > threshold:
        return [best_name, round(float(best_avg_sim) * 100, 2)]
    else:
        return ["Unknown", 0]
...
class FaceRecognitionApp(ctk.CTk):
    ...
    def start_detection(self):
        if not self.running:
            try:
                tresh_text = self.add_treshold.get()
                try:
                    self.tresh = float(tresh_text)
                except Exception:
                    print('Invalid threshold, using 0.7')
                    self.tresh = 0.7
        ...
        try:
            ...
            for img_file in image_files:
                ...
                if results.detections:
                    ...
                    if emb is not None:
                        emb_np = emb.cpu().numpy().flatten().astype('float32')
                        # نرمالایز embedding قبل از ذخیره
                        norm = np.linalg.norm(emb_np)
                        if norm > 1e-8:
                            emb_np = emb_np / norm
